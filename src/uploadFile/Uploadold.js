...
class Upload {
      ...
      `<div id="upload-wrapper" class="upload-wrapper">
          <div id="upload-text" class="upload-text">
              <div id="upload-drag-wrapper" class="upload-drag-wrapper">
                  <span id="drag-text" class="drag-text">${opts.dragText}</span>
          ...
          <div id="upload-progress-wrapper" class="upload-progress-wrapper">
              ...
              <button id="upload-btn" class="upload-btn" type="button"><span>${opts.uploadBtnText}</span></button>
  ...
  [event](opts) {
    ...
    $drag.addEventListener('drop', (e) => {
      ...
      if (this.uploading) {
        this.uploadTip(this.opts.uploadTipLoading);
        return;
      }
    ...
    $uploadBtn.onclick = async () => {
      ...
      // 表单验证
      if (!this.file) {
        this.uploadError(this.opts.uploadTipChoiceFile);
        return;
      }
      if (this.uploading) {
        this.uploadTip(this.opts.uploadTipLoading);
      ...
      if (result.status) {
        ...
        this.uploadSuccess(this.opts.uploadTipSuccess);
  ...
  async processInit() {
    ...
    this.$uploadList.innerHTML =
      this.opts.uploadTipFilename +
      '<b title="' +
      this.file.name +
      '">' +
      this.file.name +
      '</b><br>' +
      this.opts.uploadTipFileSize +
      ' <b>' +
    ...
    if (this.opts.uploadLimit && this.opts.uploadLimit < this.file.size) {
      this.uploadError(
        this.opts.uploadTipFileSizeLess +
        ' ' +
        this.getReadableSizeStr(this.opts.uploadLimit) +
        ' ' +
        this.opts.uploadTipFile
      );
      return false;
    }
    // 文件类型判断
    if (this.opts.typeLimit) {
      if (!(await this.checkFileType())) {
        this.uploadError(this.opts.uploadTipFileCheck);
        return false;
      }
    }
    ...
    this.$dragFilename.innerText = this.opts.uploadTipHandleFile + ' "' + this.file.name + '"';
  ...
  async checkFileType() {
    ...
    this.newType = this.opts.typeLimit;
    // 用户自定义的判断函数
    if (typeof this.opts.customerCheckFileType === 'function') {
      let flag = false;
      let customerCheck = await this.opts.customerCheckFileType(this.file, this.blobToString, this.opts.typeLimit);
      if (customerCheck.constructor == Object) {
        this.newType = customerCheck.newType ? customerCheck.newType : '';
        flag = customerCheck.flag ? customerCheck.flag : false;
      } else {
        flag = customerCheck;
      }
      return (
    ...
    return new Promise((resolve) => {
      ...
      if (!!window.ActiveXObject || "ActiveXObject" in window) {
        ...
        reader2.onload = function () {
          let bytes = new Uint8Array(reader2.result);
          let length = bytes.byteLength;
          let binary = '';
          for (let i = 0; i < length; i++) {
            binary += String.fromCharCode(bytes[i]);
          }
          const ret2 = binary.split('')
            .map((v) => v.charCodeAt())
  ...
  setMeregFileProgress(progress) {
    const meregCube = {
      name: 'meregfile-' + this.chunks.length,
      progress: progress
    };
  ...
  async uploadFile() {
    if (!this.file) {
      return {
        status: false,
        msg: this.opts.uploadTipChoiceFile
      };
    }
    this.process = 0;
    this.uploading = true;
    ...
    try {
      ...
      return {
        status: false,
        msg: this.opts.uploadTipFailedNetwork
      };
    }
    if (!checkData.data.data) {
      this.uploading = false;
      this.uploadTip(this.opts.uploadTipCheckFile);
      return {
        status: 0,
        msg: this.opts.uploadTipCheckFile
      };
    }
    const {
      data: {
  ...
  async sendRequest(requests) {
    if (!requests.length) {
      return {
        status: true,
        msg: this.opts.uploadTipSuccess
      };
    }
    // limit仕并发数
    // 一个数组,长度仕limit
        ...
        if (task) {
          ...
          try {
            ...
            if (e.response.status === 413) {
              ...
              resolve({
                status: false,
                msg: this.opts.uploadTipFailedSize
              });
              ...
              resolve({
                status: false,
                msg: this.opts.uploadTipFailedType
              });
              ...
              resolve({
                status: false,
                msg: this.opts.uploadTipFailedAuth
              });
            } else {
              if (task.error < 3) {
                task.error++;
                ...
                resolve({
                  status: false,
                  msg: this.opts.uploadTipFailedMore
                });
              }
            }
          }
  ...
  async mergeRequest() {
    try {
      ...
      if (!code) {
        ...
        return {
          status: true,
          msg: this.opts.uploadTipMeregSuccess,
          data: data
        };
      } else {
      ...
      if (e.response.status === 403) {
        ...
        return {
          status: false,
          msg: this.opts.uploadTipFailedHash
        };
      }
      return {
        status: false,
        msg: this.opts.uploadTipFailedUnknow
      };
    }
  }
  /**
